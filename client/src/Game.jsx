      ...
      <div className="sidebar">
        <div className="glass-panel p-4">
          ...
          <div className="flex flex-col gap-2">
            {players.map(p => (
              <div key={p.id} className="flex items-center gap-2 p-2 rounded" style={{ background: currentTurn === p.id ? 'rgba(255,255,255,0.1)' : 'transparent' }}>
        ...
        <div className="glass-panel p-4 dice-area">
          ...
          <button 
            className={`btn flex items-center justify-center gap-2 ${canRoll ? 'primary animate-pulse' : 'secondary opacity-50 cursor-not-allowed'}`}
            onClick={handleRoll}
            disabled={!canRoll}
          >
            <Dices size={20} />
          ...
          {isMyTurn && diceValue !== null && movablePieces.length === 0 && (
            <p className="text-red-400 text-sm text-center">No valid moves. Passing turn...</p>
          )}
        </div>
        ...
        <div className="chat-area glass-panel">
          ...
          <form className="chat-input" onSubmit={handleChat}>
            ...
            />
            <button type="submit" className="p-2 bg-blue-600 rounded hover:bg-blue-700 transition">
              <Send size={16} />
            </button>
          </form>
        </div>
